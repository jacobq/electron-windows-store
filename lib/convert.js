...
function convertWithFileCopy (program) {
  ...
  return emptyDir(preAppx)
    ...
    .then(() => {
      ...
      .then(() => readFile(manifestTemplate))
      .catch((err) => {
        console.error('HEY!!!!', manifestTemplate, err)
        console.log('!!!', fs.readFileSync(manifestTemplate))
        utils.debug(`Could not read manifest template. Error: ${JSON.stringify(err)}`)
        utils.log(err)
        return new Error(err)
        // FIXME: this doesn't stop the rest of the chain
      })
      .then(manifestTemplate => {
        utils.log(chalk.green.bold('Creating manifest...'))
        return writeFile(manifest, template(manifestTemplate)({
          publisherName: program.publisher, // TODO: Why not make these names match?
          publisherDisplayName: program.publisherDisplayName || 'Reserved',
          identityName: program.identityName || program.packageName,
          // TODO: document `logo` option -- see if it is redundant given `assets` options
          logo: Object.assign(['44x44', '50x50', '150x150', '310x150'].reduce((logo, size) => {
            logo[size] = path.join('assets', `SampleAppx.${size}.png`)
            return logo
          }, {}), program.logo),
